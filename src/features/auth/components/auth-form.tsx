  ...
  return (
    <div className="surface w-full max-w-md rounded-[2rem] p-6 sm:p-8">
      ...
      <form className="mt-6 space-y-4" onSubmit={submit}>
        ...
        <label className="block"><span className="mb-2 block text-xs font-semibold uppercase tracking-[.14em] text-[var(--muted)]">Password</span><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required minLength={6} autoComplete={mode === "signin" ? "current-password" : "new-password"} className="min-h-12 w-full rounded-2xl border border-white/10 bg-black/25 px-4 text-base outline-none focus:border-[var(--accent)] focus:ring-3 focus:ring-[var(--accent-glow)]" placeholder="At least 6 characters" /></label>
