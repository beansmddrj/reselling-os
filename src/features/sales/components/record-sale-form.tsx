...
export function RecordSaleForm({ candidates, initialUnitId, soldAtDefault }: { candidates: SaleCandidate[]; initialUnitId?: string; soldAtDefault: string }) {
  ...
  const [variantLabel, setVariantLabel] = useState(initial?.variants[0]?.label ?? "");
  ...
  return <form action={action} className="surface rounded-[2rem] p-5 sm:p-7">
    ...
    <div className="mt-7 grid gap-5 sm:grid-cols-2">
      <label className="sm:col-span-2"><span className="text-sm font-medium">Inventory item</span><select name="unitId" value={unitId} onChange={(event) => { const next = candidates.find((item) => item.id === event.target.value); setUnitId(event.target.value); setSalePrice(next?.askingPriceCents == null ? "" : (next.askingPriceCents / 100).toFixed(2)); setQuantity("1"); setVariantLabel(next?.variants[0]?.label ?? ""); if (next?.platform) setPlatform(next.platform); }} className={inputClass}>{candidates.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.sku} · {item.status}{item.sellMultiple ? " · repeats" : ""}</option>)}</select></label>
      {selected?.inventoryMode === "bulk" && selected.variants.length > 0 && <label><span className="text-sm font-medium">Variant</span><select required name="variantLabel" value={variantLabel} onChange={(event) => { setVariantLabel(event.target.value); setQuantity("1"); }} className={inputClass}>{selected.variants.map((variant) => <option key={variant.label} value={variant.label}>{variant.label} · {variant.availableQuantity} available</option>)}</select></label>}
      {selected?.inventoryMode === "bulk" && <label><span className="text-sm font-medium">Quantity sold</span><input required inputMode="numeric" name="quantity" min="1" max={selected.variants.length > 0 ? selected.variants.find((variant) => variant.label === variantLabel)?.availableQuantity ?? 0 : selected.availableQuantity} value={quantity} onChange={(event) => setQuantity(event.target.value)} className={inputClass}/><span className="mt-2 block text-xs text-[var(--muted)]">{selected.variants.length > 0 ? selected.variants.find((variant) => variant.label === variantLabel)?.availableQuantity ?? 0 : selected.availableQuantity} available{variantLabel ? ` in ${variantLabel}` : ""}</span></label>}
      {selected?.inventoryMode !== "bulk" && <input type="hidden" name="quantity" value="1"/>}
      {selected?.inventoryMode === "bulk" && selected.variants.length === 0 && <input type="hidden" name="variantLabel" value=""/>}
      <label><span className="text-sm font-medium">{selected?.inventoryMode === "bulk" ? "Sale total" : "Sale price"}</span><div className="relative"><span className="pointer-events-none absolute left-4 top-[1.05rem] text-[var(--muted)]">$</span><input required inputMode="decimal" name="salePrice" value={salePrice} onChange={(event) => setSalePrice(event.target.value)} placeholder="0.00" className={`${inputClass} pl-8`}/></div></label>
    ...
    {selected?.inventoryMode === "bulk" && <p className="mt-5 rounded-2xl border border-[var(--accent)]/15 bg-[var(--accent)]/[.05] px-4 py-3 text-sm text-white/80">Bulk stock is on. This records one sale transaction, reduces the exact quantity you enter{selected.variants.length > 0 ? " from the chosen variant" : ""}, and blocks overselling automatically.</p>}
    {selected?.sellMultiple && selected.inventoryMode !== "bulk" && <p className="mt-5 rounded-2xl border border-[var(--accent)]/15 bg-[var(--accent)]/[.05] px-4 py-3 text-sm text-white/80">Sell multiple is on. This sale stays under the same listing and reduces available quantity by one. Add stock from the item page when you replenish.</p>}
