...
export function RecordSaleForm({ candidates, initialUnitId, soldAtDefault }: { candidates: SaleCandidate[]; initialUnitId?: string; soldAtDefault: string }) {
  ...
  return <form action={action} className="surface rounded-[2rem] p-5 sm:p-7">
    ...
    {selected?.sellMultiple && <p className="mt-5 rounded-2xl border border-[var(--accent)]/15 bg-[var(--accent)]/[.05] px-4 py-3 text-sm text-white/80">Sell multiple is on. This sale stays under the same listing and reduces available quantity by one. Add stock from the item page when you replenish.</p>}
