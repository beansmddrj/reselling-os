...
export async function createShipmentAction(_previous: IncomingState, formData: FormData): Promise<IncomingState> {
  ...
  const { data: shipment, error } = await supabase.from("inbound_shipments").insert({ owner_id: ownerId, business_id: businessId, name, supplier_name: text(formData.get("supplier")), supplier_order_reference: text(formData.get("reference")), expected_pieces: pieces, ordered_on: date(formData.get("orderedOn")), expected_delivery_on: date(formData.get("expectedDelivery")), contents: text(formData.get("contents")), notes: text(formData.get("notes")), visibility: formData.get("visibility") === "owner_only" ? "owner_only" : "standard" }).select("id").single();
  ...
  const financial = await supabase.from("inbound_shipment_financials").insert({ shipment_id: shipment.id, owner_id: ownerId, business_id: businessId, purchase_cost_cents: purchase, shipping_cost_cents: shipping, tax_cost_cents: tax, duties_cost_cents: duties, insurance_cost_cents: insurance, other_cost_cents: other, projected_revenue_cents: projectedRevenue || null });
  if (financial.error) return { status: "error", message: financial.error.message };
  await supabase.from("business_events").insert({ owner_id: ownerId, business_id: businessId, event_type: "shipment_created", entity_type: "inbound_shipment", entity_id: shipment.id, metadata: { actor_id: ownerId } });
...
export async function addPackageAction(_previous: IncomingState, formData: FormData): Promise<IncomingState> {
  const { supabase, ownerId, businessId, role } = await getBusinessContext(); if (role !== "owner") return { status: "error", message: "Only the owner can manage shipment packages." };
  ...
  const { error } = await supabase.from("inbound_packages").insert({ shipment_id: shipmentId, owner_id: ownerId, business_id: businessId, carrier: text(formData.get("carrier")), tracking_number: text(formData.get("tracking")), tracking_url: text(formData.get("trackingUrl")), status, expected_pieces: pieces, estimated_delivery_on: date(formData.get("estimatedDelivery")), last_known_location: text(formData.get("location")) });
...
export async function receiveShipmentAction(_previous: IncomingState, formData: FormData): Promise<IncomingState> {
  const { supabase, ownerId, businessId, role } = await getBusinessContext(); if (role !== "owner") return { status: "error", message: "Only the owner can receive a shipment." };
  ...
  const { data: shipment, error: findError } = await supabase.from("inbound_shipments").select("expected_pieces, received_pieces").eq("id", shipmentId).eq("business_id", businessId).single();
  if (findError || !shipment) return { status: "error", message: "Shipment could not be found." };
  const { error } = await supabase.from("inbound_receipts").insert({ shipment_id: shipmentId, package_id: text(formData.get("packageId")), owner_id: ownerId, business_id: businessId, received_pieces: quantity, exception_type: exception || null, notes: text(formData.get("notes")) });
  ...
  const update = await supabase.from("inbound_shipments").update({ received_pieces: received, status }).eq("id", shipmentId).eq("business_id", businessId);
...
export async function updateShipmentStatusAction(formData: FormData) {
  const { supabase, businessId, role } = await getBusinessContext(); const shipmentId = String(formData.get("shipmentId")); const status = String(formData.get("status"));
  if (role !== "owner" || !statuses.includes(status)) return;
  await supabase.from("inbound_shipments").update({ status }).eq("id", shipmentId).eq("business_id", businessId);
