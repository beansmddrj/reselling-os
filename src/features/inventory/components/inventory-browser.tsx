...
type Filter = "all" | "archived" | InventoryStatus;
...
export function InventoryBrowser({ items }: { items: InventoryListItem[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-[var(--accent)]">Inventory</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.04em] sm:text-4xl">Everything you own.</h1><p className="mt-2 text-sm text-[var(--muted)]">{activeItems.length ? `${activeItems.reduce((sum, item) => sum + item.availableCount + item.soldCount, 0)} physical units grouped into ${activeItems.length} ${activeItems.length === 1 ? "item" : "items"}.` : "Physical units are the source of truth."}</p></div><Link href="/intake" className="hidden rounded-2xl bg-[var(--accent)] px-4 py-3 text-sm font-bold text-black sm:block">+ Add product</Link></div>
      ...
      {quickMenu && <div role="menu" aria-label={`Quick actions for ${quickMenu.item.name}`} onClick={(event) => event.stopPropagation()} className="fixed z-50 min-w-52 overflow-hidden rounded-2xl border border-white/10 bg-[#151820]/98 p-1.5 shadow-2xl shadow-black/60 backdrop-blur-2xl" style={{ left: Math.min(quickMenu.x, window.innerWidth - 224), top: Math.min(quickMenu.y, window.innerHeight - 300) }}><p className="truncate border-b border-white/8 px-3 py-2 text-xs font-semibold text-[var(--muted)]">{quickMenu.item.name}</p><Link role="menuitem" href={`/inventory/${quickMenu.item.id}`} className="flex min-h-11 items-center rounded-xl px-3 text-sm font-medium hover:bg-white/7">View details</Link><Link role="menuitem" href={`/inventory/${quickMenu.item.id}/edit`} className="flex min-h-11 items-center rounded-xl px-3 text-sm font-medium hover:bg-white/7">Edit item</Link><button role="menuitem" type="button" onClick={() => { void copyText(quickMenu.item.sku).then(() => { setCopiedSku(quickMenu.item.sku); window.setTimeout(() => setCopiedSku(""), 1500); setQuickMenu(null); }); }} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-medium hover:bg-white/7">Copy SKU</button><Link role="menuitem" href="/intake" className="flex min-h-11 items-center rounded-xl px-3 text-sm font-medium hover:bg-white/7">Add new product</Link><div className="my-1 border-t border-white/8"/><ArchiveInventoryButton compact unitId={quickMenu.item.id} archived={quickMenu.item.archived} onDone={() => setQuickMenu(null)}/><button role="menuitem" type="button" onClick={() => { setDeleteTarget({ id: quickMenu.item.id, name: quickMenu.item.name, status: quickMenu.item.status }); setQuickMenu(null); }} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-semibold text-red-300 hover:bg-red-400/10">Delete item…</button></div>}
