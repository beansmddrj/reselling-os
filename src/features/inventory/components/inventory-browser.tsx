...
import { DeleteInventoryDialog, type DeleteTarget } from "@/features/inventory/components/delete-inventory-dialog";
...
const restockLabels = { in_stock: "In stock", temporarily_out: "Temporarily out", restock_soon: "Restock soon", restock_asap: "Restock ASAP", do_not_restock: "Don’t restock" } as const;
...
function matchesFilter(item: InventoryListItem, value: Filter) { return value === "all" || item.status === value || (value === "sold" && item.sellMultiple && item.soldCount > 0); }
  ...
  return (
    <article onContextMenu={(event) => { event.preventDefault(); onMenu({ item, x: event.clientX, y: event.clientY }); }} className="surface group relative overflow-hidden rounded-[1.7rem] transition hover:-translate-y-0.5 hover:border-white/15">
      <Link href={`/inventory/${item.id}`} className="block focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--accent)] active:scale-[.99]">
        ...
        <div className="absolute left-3 top-3 flex flex-wrap gap-2"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] backdrop-blur-xl ${statusStyle(item.status)}`}>{item.status}</span>{item.sellMultiple && <span className="rounded-full border border-white/15 bg-black/60 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.1em] backdrop-blur-xl">{restockLabels[item.restockStatus]}</span>}</div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-3 text-[11px] text-[var(--muted)]"><span className="truncate">{item.sellMultiple ? `${item.availableCount} available · ${item.soldCount} sold` : item.storageLocation || "No location"}</span><span className="shrink-0">{date.format(new Date(item.acquiredAt))}</span></div>
        </div>
      </Link>
...
export function InventoryBrowser({ items }: { items: InventoryListItem[] }) {
  ...
  const counts = useMemo(() => Object.fromEntries(filters.map(({ value }) => [value, items.filter((item) => matchesFilter(item, value)).length])), [items]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-[var(--accent)]">Inventory</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.04em] sm:text-4xl">Everything you own.</h1><p className="mt-2 text-sm text-[var(--muted)]">{items.length ? `${items.reduce((sum, item) => sum + item.availableCount + item.soldCount, 0)} physical units grouped into ${items.length} ${items.length === 1 ? "item" : "items"}.` : "Physical units are the source of truth."}</p></div><Link href="/intake" className="hidden rounded-2xl bg-[var(--accent)] px-4 py-3 text-sm font-bold text-black sm:block">+ Add product</Link></div>
      ...
      {quickMenu && <div role="menu" aria-label={`Quick actions for ${quickMenu.item.name}`} onClick={(event) => event.stopPropagation()} className="fixed z-50 min-w-52 overflow-hidden rounded-2xl border border-white/10 bg-[#151820]/98 p-1.5 shadow-2xl shadow-black/60 backdrop-blur-2xl" style={{ left: Math.min(quickMenu.x, window.innerWidth - 224), top: Math.min(quickMenu.y, window.innerHeight - 254) }}><p className="truncate border-b border-white/8 px-3 py-2 text-xs font-semibold text-[var(--muted)]">{quickMenu.item.name}</p><Link role="menuitem" href={`/inventory/${quickMenu.item.id}`} className="flex min-h-11 items-center rounded-xl px-3 text-sm font-medium hover:bg-white/7">View details</Link><Link role="menuitem" href={`/inventory/${quickMenu.item.id}/edit`} className="flex min-h-11 items-center rounded-xl px-3 text-sm font-medium hover:bg-white/7">Edit item</Link><button role="menuitem" type="button" onClick={() => { void copyText(quickMenu.item.sku).then(() => { setCopiedSku(quickMenu.item.sku); window.setTimeout(() => setCopiedSku(""), 1500); setQuickMenu(null); }); }} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-medium hover:bg-white/7">Copy SKU</button><Link role="menuitem" href="/intake" className="flex min-h-11 items-center rounded-xl px-3 text-sm font-medium hover:bg-white/7">Add new product</Link><div className="my-1 border-t border-white/8"/><button role="menuitem" type="button" onClick={() => { setDeleteTarget({ id: quickMenu.item.id, name: quickMenu.item.name, status: quickMenu.item.status }); setQuickMenu(null); }} className="flex min-h-11 w-full items-center rounded-xl px-3 text-left text-sm font-semibold text-red-300 hover:bg-red-400/10">Delete item…</button></div>}
      ...
      <DeleteInventoryDialog target={deleteTarget} onClose={() => setDeleteTarget(null)}/>
