...
export function InventoryEditForm({ item }: { item: InventoryDetail }) {
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <section className="surface rounded-[2rem] p-5 sm:p-7"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--accent)]">Physical unit</p><div className="mt-5 grid gap-5 sm:grid-cols-2"><Field label="Item cost" name="acquisitionCost" defaultValue={(item.acquisitionCostCents / 100).toFixed(2)} required type="number" inputMode="decimal" min="0" step="0.01"/><Field label="Storage location" name="storageLocation" defaultValue={item.storageLocation}/><label className="block sm:col-span-2"><span className="mb-2 block text-xs font-semibold uppercase tracking-[.14em] text-[var(--muted)]">Restock status</span><select name="restockStatus" defaultValue={item.restockStatus} className="min-h-12 w-full rounded-2xl border border-white/10 bg-[#111319] px-4 text-base outline-none focus:border-[var(--accent)]"><option value="in_stock">In stock</option><option value="temporarily_out">Temporarily out of stock</option><option value="restock_soon">Can restock soon</option><option value="restock_asap">Restock ASAP</option><option value="do_not_restock">Don’t restock</option></select><span className="mt-2 block text-xs leading-5 text-[var(--muted)]">Used for grouped Sell multiple products. Don’t restock closes the reusable listing after its final sale.</span></label></div><p className="mt-4 text-xs text-[var(--muted)]">SKU {item.sku} and workflow status are managed separately and cannot be edited here.</p></section>
