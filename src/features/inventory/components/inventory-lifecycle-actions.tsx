...
export function InventoryLifecycleActions({ unitId, status, externalUrl }: { unitId: string; status: InventoryStatus; externalUrl: string | null }) {
  ...
  return (
    <section className="surface rounded-[2rem] p-6 sm:p-7">
      ...
      {status === "active" && <><h2 className="mt-2 text-xl font-semibold">Currently listed</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">This item stays Active until you record a sale or move it back to Ready.</p><div className="mt-5 flex flex-wrap gap-3"><Link href={`/sales/record?unitId=${unitId}`} className="grid min-h-12 place-items-center rounded-2xl bg-[var(--accent)] px-6 text-sm font-bold text-black">Record sale</Link>{externalUrl && <a href={externalUrl} target="_blank" rel="noreferrer" className="grid min-h-12 place-items-center rounded-2xl border border-white/10 px-6 text-sm font-semibold">Open listing ↗</a>}<form action={action}><input type="hidden" name="unitId" value={unitId}/><input type="hidden" name="targetStatus" value="ready"/><button disabled={pending} className="min-h-12 rounded-2xl border border-white/10 px-6 text-sm font-semibold disabled:opacity-50">{pending ? "Updating…" : "Listing ended / pause"}</button></form></div></>}
