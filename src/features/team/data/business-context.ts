...
export async function getBusinessContext() {
  ...
  const { data: memberships, error } = await supabase
    ...
    .select("business_id, business_owner_id, user_id, role, joined_at")
    .eq("user_id", userId)
    .order("joined_at");
  ...
  return { supabase, userId, businessId: membership.business_id, ownerId: membership.business_owner_id, role: membership.role };
}
