...
export async function inviteTeamMemberAction(emailInput: string) {
  ...
  const { error } = await supabase.from("business_invites").insert({ business_owner_id: ownerId, email, invited_by: userId });
...
export async function removeTeamMemberAction(userId: string) {
  ...
  const { supabase, userId: actorId, ownerId, role } = await getBusinessContext();
  if (role !== "owner" || ownerId !== actorId) return { ok: false as const, message: "Only the business owner can remove people." };
  const { error } = await supabase.from("business_members").delete().eq("business_owner_id", ownerId).eq("user_id", userId);
  if (error) return { ok: false as const, message: error.message };
  revalidatePath("/");
...
export async function cancelTeamInviteAction(inviteId: string) {
  ...
  const { error } = await supabase.from("business_invites").delete().eq("business_owner_id", ownerId).eq("id", inviteId);
  if (error) return { ok: false as const, message: error.message };
  revalidatePath("/");
