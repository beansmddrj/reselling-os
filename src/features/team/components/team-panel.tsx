...
import { acceptTeamInviteAction, cancelTeamInviteAction, inviteTeamMemberAction, removeTeamMemberAction, updateTeamMemberRoleAction } from "@/features/team/actions/manage-team";
...
export function TeamPanel({ team }: { team: TeamPanelData }) {
  ...
  return <section className="surface rounded-[2rem] p-6 sm:p-7">
    ...
    <div className="mt-6 space-y-2">{team.members.map((member) => <div key={member.userId} className="flex min-h-14 items-center justify-between gap-3 rounded-2xl bg-black/15 px-4"><div><p className="text-sm font-semibold">{member.displayName}</p><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[var(--muted)]">{member.role}</p></div>{team.role === "owner" && member.role !== "owner" && <div className="flex items-center gap-1"><button disabled={pending} onClick={() => run(() => updateTeamMemberRoleAction(member.userId, member.role === "admin" ? "member" : "admin"))} className="min-h-10 rounded-xl px-3 text-xs font-semibold text-[var(--accent)] hover:bg-[var(--accent)]/10">{member.role === "admin" ? "Remove admin" : "Make admin"}</button><button disabled={pending} onClick={() => { if (window.confirm(`Remove ${member.displayName} from the shared workspace?`)) run(() => removeTeamMemberAction(member.userId)); }} className="min-h-10 rounded-xl px-3 text-xs font-semibold text-red-300 hover:bg-red-400/10">Remove</button></div>}</div>)}</div>
