  ...
  return (
    <div className="min-h-dvh bg-[var(--bg)] text-[var(--text)]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-white/8 bg-[var(--panel)]/90 p-5 backdrop-blur-xl lg:flex lg:flex-col">
        ...
        <div className="mt-auto space-y-3"><Link href="/settings" className={`flex min-h-13 items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-semibold transition ${pathname.startsWith("/settings") ? "border-[var(--accent)]/35 bg-[var(--accent)]/10 text-white" : "border-white/8 bg-white/[.035] text-[var(--muted)] hover:bg-white/8 hover:text-white"}`}><span className="grid size-8 place-items-center rounded-xl bg-white/7">⚙</span><span>Settings</span><span className="ml-auto text-[var(--muted)]">›</span></Link><div className="rounded-3xl border border-white/8 bg-white/[.035] p-4"><p className="text-xs uppercase tracking-[.18em] text-[var(--muted)]">System</p><div className="mt-3 flex items-center gap-2 text-sm"><span className="size-2 rounded-full bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,.7)]"/>Workspace online</div></div></div>
      </aside>
