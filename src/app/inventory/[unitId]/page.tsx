    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(20rem,.8fr)]">
        ...
        <section className="surface h-fit rounded-[2rem] p-6 sm:p-7">
          ...
          <dl className="mt-7 grid grid-cols-2 gap-x-5 gap-y-6 border-t border-white/8 pt-6"><Detail label="SKU" value={item.sku}/><Detail label="Acquired" value={date.format(new Date(item.acquiredAt))}/><Detail label="Storage" value={item.storageLocation}/><Detail label="Platform" value={item.listingPlatform}/><Detail label={item.sellMultiple ? "This unit's size" : "Size / model"} value={item.sellMultiple ? item.unitSize || "N/A" : item.size}/><Detail label="Color" value={item.color}/>{item.sellMultiple && <><Detail label="Grouped units" value={`${item.availableCount} available · ${item.soldCount} sold`}/><Detail label="Restock" value={item.restockStatus.replaceAll("_", " ")}/></>}</dl>
          {item.sellMultiple && <RepeatableQuantityControls unitId={item.id} availableCount={item.availableCount} sizeBreakdown={item.availableSizeBreakdown} unsetSizeUnits={item.unsetSizeUnits}/>}
