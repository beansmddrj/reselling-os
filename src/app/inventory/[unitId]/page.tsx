    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(20rem,.8fr)]">
        ...
        <section className="surface h-fit rounded-[2rem] p-6 sm:p-7">
          <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--accent)]">Inventory unit</p><span className="rounded-full border border-[var(--accent)]/20 bg-[var(--accent)]/8 px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-[var(--accent)]">{item.status}</span></div>
      ...
      <InventoryLifecycleActions unitId={item.id} status={item.status} externalUrl={item.listing?.externalUrl ?? null}/>
