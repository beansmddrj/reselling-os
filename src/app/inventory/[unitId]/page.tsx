...
import { BulkQuantityControls } from "@/features/inventory/components/bulk-quantity-controls";
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(20rem,.8fr)]">
        ...
        <section className="surface h-fit rounded-[2rem] p-6 sm:p-7">
          <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--accent)]">Inventory unit</p><div className="flex gap-2">{item.inventoryMode === "bulk" ? <span className="rounded-full border border-[var(--accent)]/25 bg-[var(--accent)]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-[var(--accent)]">Bulk</span> : item.sellMultiple && <span className="rounded-full border border-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-white/70">Repeat</span>}<span className="rounded-full border border-[var(--accent)]/20 bg-[var(--accent)]/8 px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-[var(--accent)]">{item.status}</span></div></div>
          ...
          <dl className="mt-7 grid grid-cols-2 gap-x-5 gap-y-6 border-t border-white/8 pt-6"><Detail label="SKU" value={item.sku}/><Detail label="Acquired" value={date.format(new Date(item.acquiredAt))}/><Detail label="Storage" value={item.storageLocation}/><Detail label="Platform" value={item.listingPlatform}/><Detail label={item.sellMultiple && item.inventoryMode !== "bulk" ? "This unit's size" : "Size / model"} value={item.sellMultiple && item.inventoryMode !== "bulk" ? item.unitSize || "N/A" : item.size}/><Detail label="Color" value={item.color}/>{item.sellMultiple && <><Detail label={item.inventoryMode === "bulk" ? "Bulk stock" : "Grouped units"} value={`${item.availableCount} available · ${item.soldCount} sold`}/><Detail label="Restock" value={item.restockStatus.replaceAll("_", " ")}/></>}</dl>
          {item.inventoryMode === "bulk" && <><div className="mt-6 rounded-2xl border border-[var(--accent)]/20 bg-[var(--accent)]/[.045] p-4"><p className="text-xs font-semibold uppercase tracking-[.15em] text-[var(--accent)]">Logical bulk stock</p><p className="mt-2 text-sm leading-6 text-[var(--muted)]">This product tracks a real quantity without creating a separate inventory record for every unit. Record a sale from its active listing to reduce stock safely.</p></div>{item.bulkVariantBreakdown.length > 0 && <section className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[.15em] text-[var(--accent)]">Variant stock</p><p className="text-xs text-[var(--muted)]">Available · sold</p></div><div className="mt-3 divide-y divide-white/8">{item.bulkVariantBreakdown.map((variant) => <div key={variant.label} className="flex items-center justify-between gap-4 py-3"><p className="font-semibold">{variant.label}</p><p className="text-sm text-[var(--muted)]"><strong className="text-white">{variant.available}</strong> · {variant.sold}</p></div>)}</div></section>}<BulkQuantityControls unitId={item.id} availableCount={item.availableCount} variants={item.bulkVariantBreakdown}/></>}
          {item.sellMultiple && item.inventoryMode !== "bulk" && (
            <RepeatableQuantityControls unitId={item.id} availableCount={item.availableCount} sizeBreakdown={item.availableSizeBreakdown} unsetSizeUnits={item.unsetSizeUnits}/>
          )}
