...
Bulk products start through Bulk / Lot Intake. The operator enters one product, its total quantity, optional carton/case math, per-unit cost, and per-unit asking price. If stock needs separate counts, they can expand the simple variant builder and enter labels such as `S`, `M`, and `L` with quantities that must equal the total. Inventory detail shows available/sold counts by variant; quantity adjustments and bulk sales require the exact variant. A bulk sale records one completed transaction with a selected quantity and total sale price; the database locks and consumes available lot quantity before the sale is inserted, so it cannot oversell. Shipment receiving is the next v0.1.8 increment.
...
All inventory reads, mutations, photos, search results, and events must be constrained to the active Business tenant. Cross-business product templates or aggregated inventory are not part of the launch product.
